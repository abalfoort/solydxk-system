...
def get_local_repos():
    ...
    for line in lines:
        line = line.strip()
        matchObj = re.search(r"^deb\s+(https?:[\/a-zA-Z0-9\.\-]*).*", line)
        if matchObj:
    ...
    def save(self, replaceRepos, excludeStrings=[]):
        try:
            ...
            if os.path.exists(src):
                ...
                debian_suite = ''
                matchObj = re.search(r"debian\.org\/debian/?\s+(\S*)", ' '.join(srcList))
                if matchObj:
                        ...
                        for repo in replaceRepos:
                            if repo[0] != '' and repo[0] in line:
                                ...
                                if not skip:
                                    ...
                                    regexp = r'https*://{0}'.format(repo[0]) if repo[0][:4] != 'http' else repo[0]
                                    substr = r'http://{0}'.format(repo[1]) if repo[1][:4] != 'http' else repo[1]
