...
class SolydXKSystemSettings(object):
    ...
    def fill_treeview_fstab_partitions(self):
        ...
        for partition in self.partitions:
            ...
            in_fstab = True if partition['fstab_path'] == '/etc/fstab' else False
    ...
    def get_partition_configuration_info(self, partition, partitions, check_encryptable):
        ...
        if partition['device'] not in self.failed_mount_devices:
            ...
            for fstab_path in fstab_paths:
                ...
                regexp = r"^\s*(%s|%s|%s)\s+(\S+)" % ("UUID=%s" % partition['old_uuid'], partition['old_device'], fstab_device)
                                ...
                                if matchObj:
                                    ...
                                    if basename(device) == basename(partition['device']):
                                        ...
                                        if crypttab_keyfile_path:
                                            ...
                                            regexp = r"([0-9a-z-/]+)\s+{0}\s+".format(crypttab_keyfile_dir.replace('/', r'\/'))
                                            #print(("++++ regexp = %s" % regexp))
    ...
    def save_backports(self):
        ...
        if self.chkEnableBackports.get_active():
            ...
            debian_domain = ''
            matchObj = re.search(r"([a-z0-9\.]+)debian\.org", self.current_debian_repo)
            if matchObj:
    ...
    def get_old_kernel_packages(self):
        ...
        if kernel_packages:
            ...
            for pkg in kbuild_packages:
                if pkg: kernel_packages.append(pkg)
