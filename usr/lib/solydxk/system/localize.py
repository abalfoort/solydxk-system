...
#                    minus_list.append(loc[1].replace('_', '-'))
                if has_string_in_file(loc[1], '/etc/locale.gen'):
                    ...
                    shell_exec("sed -i '0,/^# *%(lan)s.UTF-8/{s/^# *%(lan)s.UTF-8/%(lan)s.UTF-8/}' /etc/locale.gen" % {'lan': loc[1].replace('.', r'\.')})
                else:
                    # Add the locale
        ...
        shell_exec(r"sed -i '/^# Set locale$/d' {0} && " \
                   "sed -i '/^LANG=/d' {0} && " \
                   "sed -i '/^LANGUAGE=/d' {0} && " \
        ...
        if exists(self.user_dir):
            shell_exec(r"sudo -H -u %s bash -c \"sed -i 's/Language=.*/Language=%s\.utf8/' %s/.dmrc\"" % (self.user, self.default_locale, self.user_dir))
    ...
    def localizePref(self, prefsPath):
        if exists(prefsPath):
            ...
            text = self.searchAndReplace(text, r"^user_pref.*spellchecker\.dictionary.*", mozLine, mozLine)
            
            mozLine = "user_pref(\"intl.locale.matchOS\", true);"
            text = self.searchAndReplace(text, r"^user_pref.*intl\.locale\.matchOS.*", mozLine, mozLine)
            ...
            if ff_ver < 59:
                ...
                text = self.searchAndReplace(text, r"^user_pref.*general\.useragent\.locale.*", mozLine, mozLine)
            else:
                # From FF version 59 a new variable is used
                mozLine = "user_pref(\"intl.locale.requested\", \"%s\");" % moz_lan
                text = self.searchAndReplace(text, r"^user_pref.*intl\.locale\.requested.*", mozLine, mozLine)
